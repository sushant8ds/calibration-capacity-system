...
// Create a simplified server that doesn't depend on TypeScript compilation
const simpleServerContent = `// Simplified server for deployment without TypeScript compilation
...
fs.writeFileSync(path.join(__dirname, 'server-simple-deploy.js'), simpleServerContent);
console.log('✅ Created simplified deployment server');

// Update package.json to use the simple server
const packageJsonPath = path.join(__dirname, 'package.json');
const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));

packageJson.scripts.start = 'node server-simple-deploy.js';

fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));
console.log('✅ Updated package.json to use simplified server');

console.log('');
console.log('🎉 Deployment fix complete!');
console.log('📧 Email system will work automatically');
console.log('');
console.log('🚀 Now run these commands to deploy:');
console.log('   git add .');
console.log('   git commit -m "Fix: Simplified server for deployment"');
